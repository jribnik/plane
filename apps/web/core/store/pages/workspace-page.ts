...
export class WorkspacePage extends BasePage implements TWorkspacePage {
  ...
  /**
   * @description returns the current user's role in the page's workspace.
   * Unlike ProjectPage (which resolves the highest role across the page's
   * linked projects), a global/workspace page has no linked projects, so
   * permissions resolve directly from workspace membership.
   */
  private getWorkspaceRole = computedFn((): EUserPermissions | undefined => {
    ...
    return role as EUserPermissions | undefined;
  });
   ...
   * Global wiki pages have no backend "move" endpoint (they don't belong to a
   * project to move between), so this is always false.
   */
